...
// 브라우저가 파일 내용을 서버 없이 구글 자체 뷰어로 바로 열람하도록, 이 파일에 맞는
// "임베드 주소"만 가볍게 알려줍니다 (파일 전체를 읽어 인코딩하지 않아 빠릅니다 — mimeType만 확인).
// 구글 슬라이드 원본은 슬라이드 전용 임베드(반응형, 페이지 넘김 버튼 포함)를 쓰는 게 훨씬 잘 보이고,
// 일반 PDF/문서는 각각의 구글 자체 미리보기 주소를 사용합니다.
function actionGetMaterialEmbedUrl_(p) {
  const raw = String(p.file || "").trim();
  if (!raw) return { ok: false, message: "등록된 파일이 없습니다." };
  let id = raw;
  const m = id.match(/\/d\/([^/]+)/) || id.match(/[?&]id=([^&]+)/);
  if (m) id = m[1];
  let file;
  try {
    file = DriveApp.getFileById(id);
  ...
  if (mime === "application/vnd.google-apps.presentation") {
    ...
    embedUrl = `https://drive.google.com/file/d/${id}/preview`; // PDF 등 일반 파일
  }
...
function route_(action, p) {
  switch (action) {
    ...
    case "getMaterialEmbedUrl": return actionGetMaterialEmbedUrl_(p);
